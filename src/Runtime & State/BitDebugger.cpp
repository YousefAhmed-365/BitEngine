...
void BitDebugger::DrawOverlay(const DebugDrawContext& ctx) {
    ...
    DrawRectangleRounded({(float)startX, (float)startY, (float)overlayW, (float)overlayH}, 0.05f, 12, Fade(BLACK, 0.88f));
    DrawRectangleRoundedLines({(float)startX, (float)startY, (float)overlayW, (float)overlayH}, 0.05f, 12, Fade(SKYBLUE, 0.3f));
    ...
    const char* tabs[] = { "[1] DASH", "[2] EXEC", "[3] STATE", "[4] ASSETS", "[5] LOGS" };
    int tabW = overlayW / 5;
    for (int i = 0; i < 5; ++i) {
    ...
    if (m_debugTab == 0) { // DASHBOARD
        ...
        DrawRectangleRounded({(float)rx, (float)ry, 120, 20}, 0.2f, 4, Fade(LIME, 0.15f));
