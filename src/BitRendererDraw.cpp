...
void BitRenderer::DrawBackgroundElem(UIElement& elem) {
    Color clear = elem.resolvedStyle.clearColor.value_or(Color{8,8,20,255});
    DrawRectangle(0, 0, GetScreenWidth(), GetScreenHeight(), clear);
...
void BitRenderer::DrawEntityLayerElem(UIElement& elem) {
    const auto& s   = elem.resolvedStyle;
    float escale    = s.entityScale.value_or(3.0f);
    float floatAmp  = s.floatAmplitude.value_or(10.0f);
    float floatSpd  = s.floatSpeed.value_or(2.0f);
    float shadowOp  = s.shadowOpacity.value_or(0.35f);
...
void BitRenderer::DrawVignetteElem(UIElement& elem) {
    if (m_engine.GetConfigs().enable_vignette) {
        float op = elem.resolvedStyle.opacity.value_or(0.4f);
        DrawTexturePro(m_vignette,{0,0,64,64},
            {0,0,(float)GetScreenWidth(),(float)GetScreenHeight()},{0,0},0,Fade(WHITE,op));
    }
    float screenFade = m_engine.GetScreenFadeAlpha();
    if (screenFade > 0.001f) {
        BitColor bc = m_engine.GetScreenFadeColor();
        DrawRectangle(0,0,GetScreenWidth(),GetScreenHeight(),
            {bc.r,bc.g,bc.b,(uint8_t)(screenFade*255)});
    }
}
...
void BitRenderer::DrawPanelElem(UIElement& elem) {
    if (elem.role == "name_label") {
        ...
        float tw = MeasureTextEx(f, e->name.c_str(), (float)fs, 2).x;
        ...
        DrawTextEx(f, e->name.c_str(), {r.x+padL, ty}, (float)fs, 2.0f, tc);
        return;
    }
    DrawStyledPanel(elem.computedRect, elem.resolvedStyle);
}
...
void BitRenderer::DrawTextElem(UIElement& /*elem*/) {
    // Static text elements are handled by their parent panel or inline content
}

void BitRenderer::DrawRichTextElem(UIElement& elem) {
    if (!m_engine.IsActive()) return;
    if (m_engine.IsUiHidden()) return;
    const auto& s = elem.resolvedStyle;
...
void BitRenderer::DrawCursorElem(UIElement& elem) {
    ...
    if (!ct.path.empty()) {
        ...
        if (t.id > 0) {
            float sc = anim*0.08f+1.0f;
            DrawTexturePro(t,{0,0,(float)t.width,(float)t.height},
                {bR-t.width*sc/2,bB-t.height*sc/2,t.width*sc,t.height*sc},{0,0},0,ct.tint);
            return;
        }
    }
    std::string shape = s.cursorShape.value_or("triangle");
    Color col = Fade(s.cursorColor.value_or(Color{0,210,255,255}), 0.9f);
    if (shape == "dot") {
        DrawCircle((int)bR,(int)bB,sz*(0.8f+anim*0.2f),col);
    } else if (shape == "bar") {
        DrawRectangle((int)(bR-sz*2),(int)bB,(int)(sz*2),(int)(sz*0.45f),Fade(col,(anim+1.0f)*0.5f));
    } else {
        float pulse = anim*3.0f;
        DrawTriangle({bR-sz*1.25f,bB+pulse},{bR,bB-sz*1.25f+pulse},{bR-sz*2.5f,bB-sz*1.25f+pulse},col);
    }
}
...
void BitRenderer::DrawDebugOverlay() {
    ...
    for (int i=startI;i<endI;++i) {
        ...
        std::string opStr="?";
        switch(ins.op){case BitOp::SAY:opStr="SAY";break;case BitOp::TEXT:opStr="TEXT";break;
        case BitOp::CHOICE:opStr="CHOICE";break;case BitOp::IF:opStr="IF";break;
        case BitOp::GOTO:opStr="GOTO";break;case BitOp::SET:opStr="SET";break;
        case BitOp::EVENT:opStr="EVENT";break;case BitOp::LABEL:opStr="LABEL";break;
        case BitOp::HALT:opStr="HALT";break;case BitOp::CALL:opStr="CALL";break;
        case BitOp::RETURN:opStr="RET";break;default:opStr="???";break;}
        std::string args;
